...
GST_BOILERPLATE (GstGooDecPcm, gst_goo_decpcm, GstGooAudioFilter, GST_TYPE_GOO_AUDIO_FILTER);

GST_DEBUG_CATEGORY_STATIC (gst_goo_decpcm_debug);
#define GST_CAT_DEFAULT gst_goo_decpcm_debug
...
static gboolean gst_goo_decpcm_sink_setcaps (GstPad *pad, GstCaps *caps);

/* default values */
#define CHANNELS_DEFAULT 1
#define SAMPLERATE_DEFAULT 44100
#define INPUT_BUFFERSIZE_DEFAULT 4000
...
static const GstElementDetails details =
        GST_ELEMENT_DETAILS (
                "OpenMAX PCM decoder",
                "Codec/Decoder/Audio",
                "Decodes PCM streams with OpenMAX",
                "Texas Instrument"
                );
...
static gboolean
gst_goo_decpcm_process_mode_default (GstGooAudioFilter *self, guint value)
{
...
static void
gst_goo_decpcm_dispose (GObject* object)
{
...
static void
gst_goo_decpcm_base_init (gpointer g_klass)
{
        GST_DEBUG_CATEGORY_INIT (gst_goo_decpcm_debug, "goodecpcm", 0,
                                 "OpenMAX PCM decoder element");
...
static void
gst_goo_decpcm_class_init (GstGooDecPcmClass* klass)
{
	...
	g_klass->dispose = GST_DEBUG_FUNCPTR (gst_goo_decpcm_dispose);

	/* GST GOO FILTER */
	GstGooAudioFilterClass* gst_c_klass = GST_GOO_AUDIO_FILTER_CLASS (klass);
	gst_c_klass->set_process_mode_func = GST_DEBUG_FUNCPTR (gst_goo_decpcm_process_mode_default);
	return;
}

static void
gst_goo_decpcm_init (GstGooDecPcm* self, GstGooDecPcmClass* klass)
{
	GST_DEBUG ("");

	GST_GOO_AUDIO_FILTER (self)->component = goo_component_factory_get_component
		(GST_GOO_AUDIO_FILTER (self)->factory, GOO_TI_PCM_DECODER);
	...
	{
		/* input port */
                GOO_TI_PCMDEC_GET_INPUT_PARAM (component)->nChannels = CHANNELS_DEFAULT;
	}
	...
	/* The omx component provides only a port, but the filters need two */
	GST_GOO_AUDIO_FILTER (self)->outport = goo_component_get_port (component, "output0");

	/* Setcaps functions */
	gst_pad_set_setcaps_function (GST_GOO_AUDIO_FILTER(self)->sinkpad, gst_goo_decpcm_sink_setcaps);
...
static gboolean
gst_goo_decpcm_sink_setcaps (GstPad *pad, GstCaps *caps)
{
	...
	self = GST_GOO_DECPCM (GST_PAD_PARENT (pad));
	...
	GOO_TI_PCMDEC_GET_INPUT_PARAM (component)->nSamplingRate = sample_rate;
	GOO_TI_PCMDEC_GET_INPUT_PARAM (component)->nChannels = channels;
