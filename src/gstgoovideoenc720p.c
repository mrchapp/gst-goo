...
GST_BOILERPLATE (GstGooVideoEnc720p, gst_goo_videoenc_720p, GstGooVideoFilter, GST_TYPE_GOO_VIDEO_FILTER);

GST_DEBUG_CATEGORY_STATIC (gst_goo_videoenc_720p_debug);
#define GST_CAT_DEFAULT gst_goo_videoenc_720p_debug

static GstCaps* gst_goo_videoenc_720p_transform_caps (GstGooVideoFilter* filter,
	GstPadDirection direction, GstCaps* caps);
static gboolean gst_goo_videoenc_720p_configure_caps (GstGooVideoFilter* filter,
	GstCaps* in, GstCaps* out);
static GstBuffer* gst_goo_videoenc_720p_extra_buffer_processing (GstGooVideoFilter* filter,
	GstBuffer *buffer);
...
static void
gst_goo_videoenc_720p_set_property (GObject* object, guint prop_id,
				    const GValue* value, GParamSpec* pspec)
{
	GstGooVideoEnc720p* self = GST_GOO_VIDEOENC720P (object);
...
static void
gst_goo_videoenc_720p_get_property (GObject* object, guint prop_id,
...
static void
gst_goo_videoenc_720p_class_init (GstGooVideoEnc720pClass* klass)
{
	...
	g_klass->set_property =
		GST_DEBUG_FUNCPTR (gst_goo_videoenc_720p_set_property);
	g_klass->get_property =
		GST_DEBUG_FUNCPTR (gst_goo_videoenc_720p_get_property);
	...
	gst_c_klass->transform_caps = GST_DEBUG_FUNCPTR (gst_goo_videoenc_720p_transform_caps);
	gst_c_klass->set_caps = GST_DEBUG_FUNCPTR (gst_goo_videoenc_720p_configure_caps);
	gst_c_klass->extra_buffer_processing_func = GST_DEBUG_FUNCPTR (gst_goo_videoenc_720p_extra_buffer_processing);

	return;
}

static void
gst_goo_videoenc_720p_init (GstGooVideoEnc720p* self, GstGooVideoEnc720pClass* klass)
{
	return;
}

static GstBuffer*
gst_goo_videoenc_720p_extra_buffer_processing (GstGooVideoFilter* filter, GstBuffer *buffer)
{
	GstGooVideoEnc720p *self = GST_GOO_VIDEOENC720P (filter);
	GooComponent* component = GST_GOO_VIDEO_FILTER (self)->component;
...
static GstCaps*
gst_goo_videoenc_720p_transform_caps (GstGooVideoFilter* filter,
				      GstPadDirection direction, GstCaps* caps)
{
	GstGooVideoEnc720p* self = GST_GOO_VIDEOENC720P (filter);
...
static gboolean
omx_sync (GstGooVideoEnc720p* self, guint color_format, guint width, guint height, guint framerate)
{
...
static gboolean
gst_goo_videoenc_720p_configure_caps (GstGooVideoFilter* filter,
				      GstCaps* in, GstCaps* out)
{
	GstGooVideoEnc720p* self = GST_GOO_VIDEOENC720P (filter);
