...
GST_BOILERPLATE (GstGooDecMpeg2, gst_goo_decmpeg2, GstGooVideoDec, GST_TYPE_GOO_VIDEODEC);

GST_DEBUG_CATEGORY_STATIC (gst_goo_decmpeg2_debug);
#define GST_CAT_DEFAULT gst_goo_decmpeg2_debug
...
static const GstElementDetails details =
		GST_ELEMENT_DETAILS (
							"OpenMAX MPEG2 decoder",
							"Codedc/Decoder/Video",
							"Decodes MPEG2 streams with OpenMAX",
							"Texas Instrument"
							);
...
{
	GST_DEBUG_CATEGORY_INIT (gst_goo_decmpeg2_debug, "goodecmpeg2", 0,
							"OpenMAX MPEG2 decoder element");
...
static GstBuffer*
gst_goo_decmpeg2_codec_data_processing (GstGooVideoFilter *filter, GstBuffer *buffer)
{

	GstGooDecMpeg2 *self = GST_GOO_DECMPEG2 (filter);
...
static void
gst_goo_decmpeg2_class_init (GstGooDecMpeg2Class* klass)
{
	...
	gst_c_klass->codec_data_processing_func = GST_DEBUG_FUNCPTR (gst_goo_decmpeg2_codec_data_processing);

	return;
}

static void
gst_goo_decmpeg2_init (GstGooDecMpeg2* self, GstGooDecMpeg2Class* klass)
{
	GST_DEBUG ("");

	GST_GOO_VIDEO_FILTER (self)->component = goo_component_factory_get_component
		(GST_GOO_VIDEO_FILTER (self)->factory, GOO_TI_MPEG2_DECODER);
