...
GST_BOILERPLATE (GstGooDecG711, gst_goo_decg711, GstGooAudioFilter, GST_TYPE_GOO_AUDIO_FILTER);

GST_DEBUG_CATEGORY_STATIC (gst_goo_decg711_debug);
#define GST_CAT_DEFAULT gst_goo_decg711_debug
...
static gboolean gst_goo_decg711_sink_setcaps (GstPad *pad, GstCaps *caps);
...
static const GstElementDetails details =
        GST_ELEMENT_DETAILS (
                "OpenMAX G711 decoder",
                "Codec/Decoder/Audio",
                "Decodes G711 streams with OpenMAX",
                "Texas Instrument"
                );
...
static void
gst_goo_decg711_base_init (gpointer g_klass)
{
        GST_DEBUG_CATEGORY_INIT (gst_goo_decg711_debug, "goodecg711", 0,
                                 "OpenMAX G711 decoder element");
...
static void
gst_goo_decg711_class_init (GstGooDecG711Class* klass)
{
	...
	g_klass->dispose = GST_DEBUG_FUNCPTR (gst_goo_decg711_dispose);
...
static void
gst_goo_decg711_init (GstGooDecG711* self, GstGooDecG711Class* klass)
{
	GST_DEBUG ("");

	GST_GOO_AUDIO_FILTER (self)->component = goo_component_factory_get_component
		(GST_GOO_AUDIO_FILTER (self)->factory, GOO_TI_G711_DECODER);
	...
	{
		/* input port */
                GOO_TI_G711DEC_GET_INPUT_PARAM (component)->nChannels = CHANNELS_DEFAULT;
	}
	...
	/* Setcaps functions */
	gst_pad_set_setcaps_function (GST_GOO_AUDIO_FILTER(self)->sinkpad, gst_goo_decg711_sink_setcaps);
...
static gboolean
gst_goo_decg711_sink_setcaps (GstPad *pad, GstCaps *caps)
{
	...
	self = GST_GOO_DECG711 (GST_PAD_PARENT (pad));
	...
	if (g_strrstr (gst_structure_get_name (structure), "audio/x-alaw"))
	{
		GOO_TI_G711DEC_GET_INPUT_PARAM (component)->ePCMMode = OMX_AUDIO_PCMModeALaw;
	}
	else if (g_strrstr (gst_structure_get_name (structure), "audio/x-mulaw"))
	{
