...
static const GstElementDetails details =
	GST_ELEMENT_DETAILS ("OpenMAX GSM-FR encoder",
			     "Codec/Decoder/Audio",
			     "Encodes GSM FULL RATE streams with OpenMAX",
			     "Texas Instruments"
		);
...
src_template =
	GST_STATIC_PAD_TEMPLATE ("src",
				 GST_PAD_SRC,
				 GST_PAD_ALWAYS,
				 GST_STATIC_CAPS ("audio/GSM-FR, "
						  "rate = (int) 8000, "
						  "channels = (int) 1")
		);

GST_DEBUG_CATEGORY_STATIC (gst_goo_encgsmfr_debug);

#define GST_CAT_DEFAULT gst_goo_encgsmfr_debug

#define _do_init(blah) \
	GST_DEBUG_CATEGORY_INIT (gst_goo_encgsmfr_debug, "gooencgsmfr", 0,\
	"OpenMAX GSM Full Rate element");

GST_BOILERPLATE_FULL (GstGooEncGsmFr, gst_goo_encgsmfr,
			GstElement, GST_TYPE_ELEMENT, _do_init);

static GstFlowReturn
process_output_buffer (GstGooEncGsmFr* self,
		...
		gpointer data)
{
	...
	GstGooEncGsmFr* self = GST_GOO_ENCGSMFR (
...
static void omx_sync (GstGooEncGsmFr* self)
{
	g_assert (self != NULL);
	g_assert (self->component != NULL);

	OMX_AUDIO_PARAM_GSMFRTYPE* param;
	param = GOO_TI_GSMFRENC_GET_OUTPUT_PORT_PARAM (self->component);
...
static void omx_start (GstGooEncGsmFr* self)
{
	g_assert (self != NULL);
	g_assert (self->component != NULL);
...
static void omx_stop (GstGooEncGsmFr* self)
{
	g_assert (self != NULL);
	g_assert (self->component != NULL);
...
static void omx_wait_for_done (GstGooEncGsmFr* self)
{
	g_assert (self != NULL);
...
static gboolean
gst_goo_encgsmfr_event (GstPad* pad, GstEvent* event)
{
	GstGooEncGsmFr* self = GST_GOO_ENCGSMFR (gst_pad_get_parent (pad));
...
static gboolean
gst_goo_encgsmfr_setcaps (GstPad* pad, GstCaps* caps)
{
	GstGooEncGsmFr *self = GST_GOO_ENCGSMFR (gst_pad_get_parent (pad));
...
static GstFlowReturn
gst_goo_encgsmfr_chain (GstPad* pad, GstBuffer* buffer)
{
	GstGooEncGsmFr* self = GST_GOO_ENCGSMFR (gst_pad_get_parent (pad));
...
gst_goo_encgsmfr_state_change (GstElement* element,
					GstStateChange transition)
{
	g_assert (GST_IS_GOO_ENCGSMFR (element));

	GstGooEncGsmFr* self = GST_GOO_ENCGSMFR (element);
...
static void
gst_goo_encgsmfr_set_property (GObject* object, guint prop_id,
					const GValue* value, GParamSpec* pspec)
{
	g_assert (GST_IS_GOO_ENCGSMFR (object));
	GstGooEncGsmFr* self = GST_GOO_ENCGSMFR (object);
...
static void
gst_goo_encgsmfr_get_property (GObject* object, guint prop_id,
...
static void
gst_goo_encgsmfr_dispose (GObject* object)
{
...
static void
gst_goo_encgsmfr_base_init (gpointer g_klass)
{
...
static void
gst_goo_encgsmfr_class_init (GstGooEncGsmFrClass* klass)
{
	GObjectClass* g_klass = G_OBJECT_CLASS (klass);
	GstElementClass* gst_klass = GST_ELEMENT_CLASS (klass);

	g_klass->set_property = gst_goo_encgsmfr_set_property;
	g_klass->get_property = gst_goo_encgsmfr_get_property;
	g_klass->dispose = gst_goo_encgsmfr_dispose;

	gst_klass->change_state =
		GST_DEBUG_FUNCPTR (gst_goo_encgsmfr_state_change);
...
static void
gst_goo_encgsmfr_init (GstGooEncGsmFr* self, GstGooEncGsmFrClass* klass)
{
	/* goo stuff */
	...
	gst_pad_set_setcaps_function
		(self->sinkpad, GST_DEBUG_FUNCPTR (gst_goo_encgsmfr_setcaps));
	gst_pad_set_event_function
		(self->sinkpad, GST_DEBUG_FUNCPTR (gst_goo_encgsmfr_event));
	gst_pad_set_chain_function
		(self->sinkpad, GST_DEBUG_FUNCPTR (gst_goo_encgsmfr_chain));
