...
static const GstElementDetails details =
		GST_ELEMENT_DETAILS (
				"OpenMAX GSM Half Rate decoder",
				"Codec/Decoder/Audio",
				"Decodes GSM Half Rate streams with OpenMAX",
				"Texas Instruments"
				);
...
GST_DEBUG_CATEGORY_STATIC (gst_goo_decgsmhr_debug);
#define GST_CAT_DEFAULT gst_goo_decgsmhr_debug

#define _do_init(blah) \
	GST_DEBUG_CATEGORY_INIT (gst_goo_decgsmhr_debug, "goodecgsmhr", 0, "OpenMAX GSM Half Rate element");

GST_BOILERPLATE_FULL (GstGooDecGsmHr, gst_goo_decgsmhr,
		      GstElement, GST_TYPE_ELEMENT, _do_init);

static GstFlowReturn
process_output_buffer (GstGooDecGsmHr* self, OMX_BUFFERHEADERTYPE* buffer)
{
	GstBuffer* out = NULL;
	GstFlowReturn ret = GST_FLOW_ERROR;
	buffer->nFilledLen = 320;
	...
	test = buffer->nFilledLen * 8;
	test1 = self->channels * self->rate * 16;
	...
	gpointer data)
{
	...
	GstGooDecGsmHr* self = GST_GOO_DECGSMHR (
...
static void
omx_wait_for_done (GstGooDecGsmHr* self)
{
	g_assert (self != NULL);
...
static gboolean
gst_goo_decgsmhr_bytepos_to_time (GstGooDecGsmHr *self,
...
static gboolean
gst_goo_decgsmhr_event (GstPad* pad, GstEvent* event)
{
	GstGooDecGsmHr* self = GST_GOO_DECGSMHR (gst_pad_get_parent (pad));
		...
		case GST_EVENT_NEWSEGMENT:
		{
			gdouble rate, applied_rate;
      		...
      		if (format == GST_FORMAT_BYTES)
      		{
        		...
        		if (!gst_goo_decgsmhr_bytepos_to_time (self, stop, &seg_stop))
          			seg_stop = GST_CLOCK_TIME_NONE;
        		if (gst_goo_decgsmhr_bytepos_to_time (self, start, &seg_start) &&
            		gst_goo_decgsmhr_bytepos_to_time (self, pos, &seg_pos))
        		{
...
static gboolean
gst_goo_decgsmhr_setcaps (GstPad* pad, GstCaps* caps)
{
	GstGooDecGsmHr *self = GST_GOO_DECGSMHR (gst_pad_get_parent (pad));
	...
	if (self->channels != 1)
	{
		g_warning ("goodecgsmhr is only optimized for mono channels");
	}

	if (self->rate != 8000)
	{
		g_warning ("goodecgsmhr is only optimized for 8000 Hz");
	}
...
static GstFlowReturn
gst_goo_decgsmhr_chain (GstPad* pad, GstBuffer* buffer)
{
	GstGooDecGsmHr* self = GST_GOO_DECGSMHR (gst_pad_get_parent (pad));
	...
	if (goo_port_is_tunneled (self->inport))
	{
		...
		//GST_DEBUG_OBJECT (process_output_buffer, "process_output_buffer");
		goto done;
	}
	...
	while (gst_goo_adapter_available (self->adapter) >= omxbufsiz)
	{
		...
		//GST_DEBUG_OBJECT(omxbuf->nFilledLen,"omxbuf->nFilledLen");
...
gst_goo_decgsmhr_state_change (GstElement* element, GstStateChange transition)
{
	g_assert (GST_IS_GOO_DECGSMHR (element));

	GstGooDecGsmHr* self = GST_GOO_DECGSMHR (element);
...
static void
gst_goo_decgsmhr_set_property (GObject* object, guint prop_id,
			       const GValue* value, GParamSpec* pspec)
{
	g_assert (GST_IS_GOO_DECGSMHR (object));
	GstGooDecGsmHr* self = GST_GOO_DECGSMHR (object);
...
static void
gst_goo_decgsmhr_get_property (GObject* object, guint prop_id,
...
static void
gst_goo_decgsmhr_base_init (gpointer g_klass)
{
...
static void
gst_goo_decgsmhr_class_init (GstGooDecGsmHrClass* klass)
{
	GObjectClass* g_klass = G_OBJECT_CLASS (klass);
	GstElementClass* gst_klass = GST_ELEMENT_CLASS (klass);

	g_klass->set_property = gst_goo_decgsmhr_set_property;
	g_klass->get_property = gst_goo_decgsmhr_get_property;
	g_klass->dispose = gst_goo_decgsmhr_dispose;

	gst_klass->change_state =
		GST_DEBUG_FUNCPTR (gst_goo_decgsmhr_state_change);
...
static void
gst_goo_decgsmhr_init (GstGooDecGsmHr* self, GstGooDecGsmHrClass* klass)
{
	/* goo stuff */
	...
	gst_pad_set_setcaps_function
		(self->sinkpad, GST_DEBUG_FUNCPTR (gst_goo_decgsmhr_setcaps));
	gst_pad_set_event_function
		(self->sinkpad, GST_DEBUG_FUNCPTR (gst_goo_decgsmhr_event));
	gst_pad_set_chain_function
		(self->sinkpad, GST_DEBUG_FUNCPTR (gst_goo_decgsmhr_chain));
