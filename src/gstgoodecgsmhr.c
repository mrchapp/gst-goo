...
GST_BOILERPLATE (GstGooDecGsmHr, gst_goo_decgsmhr, GstGooAudioFilter, GST_TYPE_GOO_AUDIO_FILTER);
...
static gboolean gst_goo_decgsmhr_sink_setcaps (GstPad *pad, GstCaps *caps);
static gboolean gst_goo_decgsmhr_src_setcaps (GstPad *pad, GstCaps *caps);

#define GST_GOO_DECGSMHR_GET_PRIVATE(obj) \
    (G_TYPE_INSTANCE_GET_PRIVATE ((obj), GST_TYPE_GOO_DECGSMHR, GstGooDecGsmFrPrivate))
...
{
    GST_DEBUG_CATEGORY_INIT (gst_goo_decgsmhr_debug, "gooodecgsmhr", 0,
                                "OpenMAX GSMHR decoder element");
...
static gboolean
gst_goo_decgsmhr_check_fixed_src_caps (GstGooAudioFilter *filter)
{
    GstGooDecGsmHr *self = GST_GOO_DECGSMHR (filter);
    OMX_AUDIO_PARAM_PCMMODETYPE *param;
    GstCaps *src_caps;

    param = GOO_TI_GSMHRDEC_GET_OUTPUT_PORT_PARAM (GST_GOO_AUDIO_FILTER (self)->component);
...
gst_goo_decgsmhr_class_init (GstGooDecGsmHrClass *klass)
{
    ...
    /* gobject */
    g_klass->dispose = GST_DEBUG_FUNCPTR (gst_goo_decgsmhr_dispose);
...
gst_goo_decgsmhr_init (GstGooDecGsmHr *self, GstGooDecGsmHrClass *klass)
{
    ...
    GST_GOO_AUDIO_FILTER (self)->component = goo_component_factory_get_component
        (GST_GOO_AUDIO_FILTER (self)->factory, GOO_TI_GSMHR_DECODER);

    GooComponent *component = GST_GOO_AUDIO_FILTER (self)->component;

    /* component default parameters */
    {
        GOO_TI_GSMHRDEC_GET_OUTPUT_PORT_PARAM (component)->nChannels = CHANNELS_DEFAULT;
        GOO_TI_GSMHRDEC_GET_OUTPUT_PORT_PARAM (component)->nSamplingRate = DEFAULT_BITRATE;
    }
    ...
    /* setcap functions */
    gst_pad_set_setcaps_function (GST_GOO_AUDIO_FILTER (self)->sinkpad, gst_goo_decgsmhr_sink_setcaps);
    gst_pad_set_setcaps_function (GST_GOO_AUDIO_FILTER (self)->srcpad, gst_goo_decgsmhr_src_setcaps);
...
static gboolean
gst_goo_decgsmhr_sink_setcaps (GstPad *pad, GstCaps *caps)
{
    ...
    self = GST_GOO_DECGSMHR (GST_PAD_PARENT (pad));
...
static gboolean
gst_goo_decgsmhr_src_setcaps (GstPad *pad, GstCaps *caps)
{
    GstStructure *structure;
    GstGooDecGsmHr *self;
    ...
    if (comp_res == 0)
    {
        GST_DEBUG_OBJECT (self, "DASF-SINK Activated: GSM-HR Dec");
        GST_GOO_AUDIO_FILTER (self)->dasf_mode = TRUE;
    }
    else
    {
        GST_DEBUG_OBJECT (self, "FILE-TO-FILE Activated: GSM-HR Dec");
