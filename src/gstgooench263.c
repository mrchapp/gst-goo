...
GST_BOILERPLATE (GstGooEncH263, gst_goo_ench263, GstGooVideoEnc, GST_TYPE_GOO_VIDEOENC);

GST_DEBUG_CATEGORY_STATIC (gst_goo_ench263_debug);
#define GST_CAT_DEFAULT gst_goo_ench263_debug
...
{
	GstGooEncH263* self = GST_GOO_ENCH263 (object);
...
static void
gst_goo_ench263_get_property (GObject* object, guint prop_id,
...
static void
gst_goo_ench263_base_init (gpointer g_klass)
{
        GST_DEBUG_CATEGORY_INIT (gst_goo_ench263_debug, "gooench263", 0,
                                 "OpenMAX H263 encoder element");
...
static void
gst_goo_ench263_class_init (GstGooEncH263Class* klass)
{
	...
	/* gobject */
        ...
        g_klass->set_property =
                GST_DEBUG_FUNCPTR (gst_goo_ench263_set_property);
        g_klass->get_property =
                GST_DEBUG_FUNCPTR (gst_goo_ench263_get_property);
...
static void
gst_goo_ench263_init (GstGooEncH263* self, GstGooEncH263Class* klass)
{
	GST_DEBUG ("");

	GST_GOO_VIDEO_FILTER (self)->component = goo_component_factory_get_component
		(GST_GOO_VIDEO_FILTER (self)->factory, GOO_TI_H263_ENCODER);
	...
	/* output port */
	{
		...
		GOO_PORT_GET_DEFINITION (port)->format.video.cMIMEType = "263";
		GOO_PORT_GET_DEFINITION (port)->format.video.eCompressionFormat = OMX_VIDEO_CodingH263;
