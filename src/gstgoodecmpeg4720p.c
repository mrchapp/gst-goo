...
GST_BOILERPLATE (GstGooDecMpeg4720p, gst_goo_decmpeg4_720p, GstGooVideoDec720p, GST_TYPE_GOO_VIDEODEC720P);

GST_DEBUG_CATEGORY_STATIC (gst_goo_decmpeg4_720p_debug);
#define GST_CAT_DEFAULT gst_goo_decmpeg4_720p_debug
...
static const GstElementDetails details =
	GST_ELEMENT_DETAILS (
						"OpenMAX MPEG4 Ittiam decoder",
						"Codedc/Decoder/Video",
						"Decodes MPEG4 720p streams with OpenMAX",
						"Texas Instruments"
						);
...
static void
gst_goo_decmpeg4_720p_base_init (gpointer g_klass)
{
	GST_DEBUG_CATEGORY_INIT (gst_goo_decmpeg4_720p_debug, "goodecmpeg4720p", 0,
							"OpenMAX MPEG4 720p decoder element");
...
static GstBuffer*
gst_goo_decmpeg4_720p_codec_data_processing (GstGooVideoFilter *filter, GstBuffer *buffer)
{

	GstGooDecMpeg4720p *self = GST_GOO_DECMPEG4720P (filter);

	if (GST_IS_BUFFER (GST_GOO_VIDEODEC720P(self)->video_header))
	{
		GST_DEBUG_OBJECT (self, "Adding MPEG-4 header info to buffer");

		GstBuffer *new_buf = gst_buffer_merge (GST_BUFFER (GST_GOO_VIDEODEC720P(self)->video_header), GST_BUFFER (buffer));
		...
		gst_buffer_unref (GST_GOO_VIDEODEC720P(self)->video_header);
	}
...
static void
gst_goo_decmpeg4_720p_class_init (GstGooDecMpeg4720pClass* klass)
{
	...
	gst_c_klass->codec_data_processing_func = GST_DEBUG_FUNCPTR (gst_goo_decmpeg4_720p_codec_data_processing);

	return;
}

static void
gst_goo_decmpeg4_720p_init (GstGooDecMpeg4720p* self, GstGooDecMpeg4720pClass* klass)
{
	GST_DEBUG ("");

	GST_GOO_VIDEO_FILTER (self)->component = goo_component_factory_get_component
		(GST_GOO_VIDEO_FILTER (self)->factory, GOO_TI_MPEG4_720P_DECODER);
