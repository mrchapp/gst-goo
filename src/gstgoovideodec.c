...
GST_BOILERPLATE (GstGooVideoDec, gst_goo_videodec, GstGooVideoFilter, GST_TYPE_GOO_VIDEO_FILTER);

GST_DEBUG_CATEGORY_STATIC (gst_goo_videodec_debug);
#define GST_CAT_DEFAULT gst_goo_videodec_debug

static GstCaps* gst_goo_videodec_transform_caps (GstGooVideoFilter* filter,
	GstPadDirection direction, GstCaps* caps);
static gboolean gst_goo_videodec_configure_caps (GstGooVideoFilter* filter,
...
static void
gst_goo_videodec_base_init (gpointer g_klass)
{
	GST_DEBUG_CATEGORY_INIT (gst_goo_videodec_debug, "goovideodec", 0,
...
static gboolean
gst_goo_videodec_set_process_mode (GstGooVideoFilter* filter, guint value)
{

	GstGooVideoDec* self = GST_GOO_VIDEODEC (filter);
...
static void
gst_goo_videodec_class_init (GstGooVideoDecClass* klass)
{
	...
	gst_c_klass->set_process_mode_func = GST_DEBUG_FUNCPTR (gst_goo_videodec_set_process_mode);
	gst_c_klass->transform_caps = GST_DEBUG_FUNCPTR (gst_goo_videodec_transform_caps);
	gst_c_klass->set_caps = GST_DEBUG_FUNCPTR (gst_goo_videodec_configure_caps);

	return;
}

static void
gst_goo_videodec_init (GstGooVideoDec* self, GstGooVideoDecClass* klass)
{
	return;
}
...
static void
parse_in_caps (GstCaps* caps, GstGooVideoDec* self)
{
	GstStructure *structure;
	guint32 fourcc;
...
static GstCaps*
gst_goo_videodec_transform_caps (GstGooVideoFilter* filter,
				 GstPadDirection direction, GstCaps* caps)
{
	GstGooVideoDec* self = GST_GOO_VIDEODEC(filter);
...
static gboolean
gst_goo_videodec_configure_caps (GstGooVideoFilter* filter,
				  GstCaps* in, GstCaps* out)
{
	GstGooVideoDec* self = GST_GOO_VIDEODEC (filter);
